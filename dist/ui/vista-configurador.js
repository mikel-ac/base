...
export function montarConfigurador(ctx, nav) {
    ...
    function pintar(s) {
        if (!usuario)
            return;
        const u = usuario;
        const chip = (dataset, valor, etiqueta, activo) => `<button class="chip ${activo ? "on" : ""}" data-${dataset}="${valor}">${etiqueta}</button>`;
        const eleccionNivel = s.nivelDia === null ? "perfil" : s.nivelDia < u.nivel ? "suave" : s.nivelDia > u.nivel ? "fuerte" : "perfil";
        const abierto = molestiasAbierto || s.molestiasHoy.length > 0;
        const filaStepper = (campo, etiqueta, unidad) => `
      <div class="frow">
      ...
      <div>
        ...
        <div class="chips">
          ${Object.keys(ETIQUETA_ZONA_TRABAJO)
    ...
    function alPulsar(ev) {
        ...
        if (d["focus"]) {
            ...
            return configurador.alternarFocusSil(d["focus"]);
        }
        if (d["zonaTrabajo"])
            return configurador.fijarZonaTrabajo(d["zonaTrabajo"]);
        if (d["material"]) {
            boton.classList.toggle("on");
            return configurador.alternarMaterialSil(d["material"]);
        }
        if (d["zona"]) {
            boton.classList.toggle("on");
            return configurador.alternarMolestiaHoySil(d["zona"]);
        }
        if (d["nivel"]) {
            ...
            if (d["nivel"] === "suave")
                configurador.fijarNivelDiaSil(nivelSuave(u));
            else if (d["nivel"] === "perfil")
                configurador.fijarNivelDiaSil(null);
            else if (d["nivel"] === "fuerte")
                configurador.fijarNivelDiaSil(nivelFuerte(u));
            return;
        }
        if (d["tiempo"]) {
            const campo = d["tiempo"];
