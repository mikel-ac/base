 ...
 */
export function mostrarDetallePlan(catalogo, planInicial, titulo, alEmpezar) {
    let plan = {
        ...planInicial,
    ...
    function sustituir(bloque, i) {
        ...
        abrirSelectorSustitucion(actual.ejercicio.nombre, todos, (elegido) => {
            ...
            render();
            const fila = velo.querySelector(`li[data-bloque="${bloque}"][data-i="${i}"]`);
            if (fila) {
    ...
    velo.addEventListener("click", (ev) => {
        ...
        if (objetivo === velo) {
            ...
            if (gesto.fueArrastre(ev))
                return;
            pedirCierre();
            return;
        }
        const sust = objetivo.closest("[data-sust]");
        if (sust) {
            const partes = sust.dataset["sust"].split(":");
            sustituir(partes[0], Number(partes[1]));
            return;
        }
