 ...
 */
export function mostrarDetallePlan(catalogo, plan, titulo, alEmpezar) {
    const todos = [...plan.calentamiento, ...plan.principal];
    const item = (a) => `<li data-indice="${todos.indexOf(a)}">${esc(a.ejercicio.nombre)} <span class="var">· ${esc(a.variante.nombre)}</span></li>`;
    ...
    velo.addEventListener("click", (ev) => {
        ...
        if (li) {
            ...
            if (asignado)
                mostrarDetalleEjercicio(asignado);
        }
    });
