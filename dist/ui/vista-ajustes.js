...
export function montarAjustes(ctx, nav) {
    const { raiz, app } = ctx;
    let usuario = null;
    let animado = false;
    ...
    function pintar() {
        ...
        const chipsMolestias = TODAS_ZONAS.map((z) => `<button class="chip ${u.molestiasPermanentes.includes(z) ? "on" : ""}" data-zona="${z}">${ETIQUETA_ZONA[z]}</button>`).join("");
        const segTema = ["sistema", "claro", "oscuro"]
    ...
    `;
        ...
        const campoNombre = raiz.querySelector("#nombre");
