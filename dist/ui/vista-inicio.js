...
function diaCorto(ts) {
    return new Date(ts).toLocaleDateString("es-ES", { weekday: "short" });
}
function textoObjetivo(hechas, objetivo) {
    if (hechas === 0)
        return "Cuando quieras. Mejor algo que nada.";
    if (hechas < objetivo)
        return "Vas bien. Una más y semana cumplida.";
    return "Semana cumplida. Lo que hagas de más, regalo.";
}
function tituloSugerencia(s) {
    if (s.focus.length >= 3)
        return "Sesión equilibrada";
    const texto = s.focus.join(" + ");
    return texto.charAt(0).toUpperCase() + texto.slice(1);
}
export function montarInicio(ctx, nav) {
    ...
    `;
        ...
        const puntitos = raiz.querySelectorAll(".dots i");
        puntitos.forEach((punto, i) => {
            if (i < m.sesionesEstaSemana)
                window.setTimeout(() => punto.classList.add("f"), 350 + i * 140);
        });
    }
    function generarDesdeSugerencia() {
    ...
    function alPulsar(ev) {
        ...
        if (!boton)
            return;
        if (manejarNav(boton, nav, "inicio"))
            return;
        ...
        switch (boton.dataset["accion"]) {
            ...
            case "detalle-sugerencia": {
                const plan = generarDesdeSugerencia();
                if (plan)
                    mostrarDetallePlan(catalogo, plan, "Sugerencia de hoy", (p) => nav.aSesion(p));
                break;
            }
            case "empezar-rapido": {
