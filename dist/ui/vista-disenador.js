...
export function montarDisenador(ctx, nav, planExistente) {
    const { raiz, app } = ctx;
    ...
    function huecos(linea) {
        const e = porId.get(linea.ejercicioId);
        return e?.porLados ? 2 : 1;
    }
    function totalHuecos() {
        const cuenta = (arr) => arr.reduce((s, l) => s + huecos(l), 0);
    ...
    function idsEnUso() {
        const s = new Set();
        for (const l of [...calentamiento, ...principal])
            s.add(l.ejercicioId);
        return s;
    }
    // ---------- pintado de una fila de ejercicio del entrenamiento ----------
    function filaLinea(b, linea, indice) {
        const e = porId.get(linea.ejercicioId);
    ...
    function refrescarBloques() {
        const cal = raiz.querySelector("#ds-cal");
        const pri = raiz.querySelector("#ds-prin");
        const tot = raiz.querySelector("#ds-total");
        if (cal)
            cal.innerHTML = htmlBloque("calentamiento", "Calentamiento");
        if (pri)
            pri.innerHTML = htmlBloque("principal", "Principal");
        if (tot)
            tot.textContent = `~${minutosEstimados()} min`;
    }
    // ---------- selector de ejercicios (overlay) ----------
    function candidatosSelector() {
    ...
    function pintarSelector() {
      ...
      <div class="panel ds-selector" role="dialog" aria-label="Añadir ejercicio">
        ...
        const buscar = velo.querySelector("#ds-selbuscar");
        buscar?.addEventListener("input", () => {
            selBusca = buscar.value;
            // Repintar solo la lista para no perder el foco del buscador.
            const cont = velo.querySelector(".ds-candlista");
            if (!cont)
                return;
        ...
        velo.addEventListener("click", (ev) => {
            ...
            const cand = objetivo.closest("[data-cand]");
            if (cand) {
            ...
            const chip = objetivo.closest("[data-selfiltro]");
            if (chip) {
    ...
    let arrastre = null;
    function iniciarArrastre(ev, asa) {
        const fila = asa.closest(".ds-item");
        if (!fila)
            return;
        const b = fila.dataset["bloque"];
        const contenedor = raiz.querySelector(`.ds-lista[data-lista="${b}"]`);
        if (!contenedor)
            return;
    ...
    function moverArrastre(ev) {
        ...
        const filas = Array.from(arrastre.contenedor.querySelectorAll(".ds-item:not(.ds-arrastrando)"));
        ...
        if (insertarAntesDe)
            arrastre.contenedor.insertBefore(arrastre.fila, insertarAntesDe);
        else
            arrastre.contenedor.appendChild(arrastre.fila);
    }
    function soltarArrastre() {
        ...
        const filas = Array.from(contenedor.querySelectorAll(".ds-item"));
        ...
        if (movida)
            arr.splice(hasta, 0, movida);
        refrescarBloques();
    }
    // ---------- steppers ----------
    function ajustarTiempo(campo, dir) {
        ...
        if (campo === "workSec")
            ...
            restSec = nuevo;
        const spanW = raiz.querySelector("#ds-work");
        const spanR = raiz.querySelector("#ds-rest");
        if (spanW)
            spanW.textContent = `${workSec} s`;
        if (spanR)
            spanR.textContent = `${restSec} s`;
        const tot = raiz.querySelector("#ds-total");
        if (tot)
            tot.textContent = `~${minutosEstimados()} min`;
    }
    ...
    async function guardar() {
        ...
        const campoNombre = raiz.querySelector("#ds-nombre");
    ...
    function alPulsar(ev) {
        const objetivo = ev.target;
        const stepper = objetivo.closest("[data-tiempo]");
        if (stepper) {
            ajustarTiempo(stepper.dataset["tiempo"], stepper.dataset["dir"]);
            return;
        }
        ...
        if (quitar) {
            const fila = quitar.closest(".ds-item");
            if (fila) {
                const b = fila.dataset["bloque"];
        ...
        const accion = objetivo.closest("[data-accion]")?.dataset["accion"];
        if (accion === "volver") {
    ...
    function alPointerDown(ev) {
        ...
        if (asa)
            iniciarArrastre(ev, asa);
    }
    function alNombreInput(ev) {
