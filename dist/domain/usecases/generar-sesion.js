...
import { barajar, clamp, rngPorDefecto } from "../../core/util.js";
...
export function numEjerciciosPara(durMin, workSec, restSec) {
    const intervalo = workSec + restSec;
    if (intervalo <= 0 || durMin <= 0)
        return 0;
 ...
 */
export function numCalentamientos(calentamientoMin) {
 ...
 */
export function seleccionBalanceada(pool, n, rng, enfasis) {
    ...
    const recargar = () => {
        for (const [p, lista] of grupos)
            restantes.set(p, [...lista]);
    };
    recargar();
    const resultado = [];
    const usosPorPatron = new Map();
    ...
    while (resultado.length < n) {
        ...
        const soloQuedaElUltimo = candidatos.length === 1 && candidatos[0] === ultimoPatron && grupos.size > 1;
        ...
        if (conMaterial >= capMaterial && hayPesoCorporal) {
            const soloPesoCorporal = (lista) => lista.filter((p) => restantes.get(p).some((e) => e.materiales.length === 0));
        ...
        const uso = (p) => usosPorPatron.get(p) ?? 0;
        ...
        if (elegido.materiales.length > 0)
            conMaterial++;
...
function elegirCalentamiento(catalogo, cfg, rng) {
 ...
 */
export function generarSesion(catalogo, cfg, rng = rngPorDefecto) {
    const focus = cfg.focus.filter((f) => f !== "calentamiento");
    if (focus.length === 0) {
        return fallo("Elige al menos un enfoque.");
    }
    const cfgLimpia = { ...cfg, focus };
    ...
    for (const e of seleccion) {
        ...
        if (expandidos.length + necesita > n && expandidos.length > 0)
            continue;
        if (e.porLados)
            expandidos.push(e, e);
        else
            expandidos.push(e);
        if (expandidos.length >= n)
            break;
    }
