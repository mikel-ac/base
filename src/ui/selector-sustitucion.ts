 ...
 */
export function abrirSelectorSustitucion(
  nombreActual: string,
  candidatos: Sustituto[],
  alElegir: (elegido: Sustituto) => void
): void {
  let inicio = 0;
  const PORPAGINA = 3;
  ...
  velo.addEventListener("click", (ev) => {
    ...
    if (objetivo.closest("[data-accion='otras']")) {
      inicio += PORPAGINA;
      if (inicio >= candidatos.length) inicio = 0; // vuelve al principio
      render();
      return;
    }
    const elegir = objetivo.closest<HTMLElement>("[data-elegir]");
    if (elegir) {
