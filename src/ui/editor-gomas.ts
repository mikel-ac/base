...
export function mostrarEditorGomas(alGuardar?: () => void): void {
  ...
  velo.addEventListener("click", (ev) => {
    ...
    const mover = objetivo.closest<HTMLElement>("[data-mover]");
    if (mover) {
      capturar();
      const fila = mover.closest<HTMLElement>(".goma-edit-fila")!;
    ...
    if (borrar) {
      capturar();
      const fila = borrar.closest<HTMLElement>(".goma-edit-fila")!;
      const i = Number(fila.dataset["i"]);
      lista.splice(i, 1);
      render();
      return;
