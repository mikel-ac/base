 ...
 */
export function mostrarListaSesion(
  pasos: PasoRunner[],
  indiceActual: number,
  alCerrar?: () => void
): void {
  const item = (p: PasoRunner, i: number): string => {
  ...
  const bloque = (nombre: "calentamiento" | "principal"): string =>
    pasos
      .map((p, i) => ({ p, i }))
  ...
  velo.addEventListener("click", (ev) => {
    ...
    if (li) {
      ...
      if (paso) mostrarDetalleEjercicio(paso.asignado); // se abre encima; la lista sigue detrás
    }
  });
