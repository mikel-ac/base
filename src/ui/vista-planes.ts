...
function cfgParaUsar(cfgBase: ConfigSesion, usuario: Usuario): ConfigSesion {
...
const ETIQUETA_ZONA_TRABAJO: Record<string, string> = {
  core: "core",
  pierna_gluteo: "pierna y glúteo",
  tren_superior: "tren superior",
};

function resumenPlan(p: PlanGuardado): string {
  ...
  if (zona !== "todo") partes.push(ETIQUETA_ZONA_TRABAJO[zona]!);
  partes.push(cfg.focus.join(" y "), `${cfg.workSec}/${cfg.restSec}s`);
  if (cfg.bajoImpacto) partes.push("bajo impacto");
  return partes.join(" · ");
}

export function montarPlanes(ctx: Ctx, nav: Nav): () => void {
  const { raiz, app, catalogo } = ctx;
  let usuario: Usuario | null = null;
  let planesPorId = new Map<string, PlanGuardado>();
  let animado = false;

  function pintar(estado: PlanesState): void {
    if (estado.cargando) {
  ...
  function alPulsar(ev: Event): void {
    ...
    const u = usuario;
    if (manejarNav(boton, nav, "planes")) return;
    ...
    switch (boton.dataset["accion"]) {
      ...
      case "usar": {
        ...
        if (esEntrenamientoFijo(plan)) {
          ...
          if (res.ok) mostrarDetallePlan(catalogo, res.valor, `Plan: ${plan.nombre}`, (p) => nav.aSesion(p));
