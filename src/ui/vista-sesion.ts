...
function sonarEfectos(efectos: EfectoRunner[]): void {
  for (const efecto of efectos) {
    ...
    if (efecto === "AVISO_TRANS_CAL") pitido(440, 0.12);
    if (efecto === "AVISO_FIN") {
      pitido(660, 0.15, 0);
      pitido(660, 0.15, 0.2);
      pitido(880, 0.3, 0.4);
...
function duracionDeFase(s: RunnerState): number {
  ...
  if (s.fase === "transicion-cal") return s.transCalSec;
...
async function pedirWake(): Promise<void> {
  try {
    ...
    if (!n.wakeLock) return; // no soportado: no hay nada que hacer
    if (wakeLock) return; // ya tenemos uno vivo
...
function soltarWake(): void {
  try { void wakeLock?.release(); } catch { /* nada */ }
  wakeLock = null;
}
 ...
 */
let videoDespierta: HTMLVideoElement | null = null;
...
function activarVideoDespierta(): void {
  if (videoDespierta) return;
  ...
  void v.play().catch(() => { /* si no arranca, no pasa nada */ });
  videoDespierta = v;
}
function pararVideoDespierta(): void {
  if (!videoDespierta) return;
  try { videoDespierta.pause(); videoDespierta.remove(); } catch { /* nada */ }
  videoDespierta = null;
}

/** ¿El navegador soporta la Wake Lock API? */
function hayWakeLockNativo(): boolean {
...
export function leerSesionActiva(): { plan: PlanSesion; estado: RunnerState } | null {
  try {
    ...
    if (typeof d.estado.transCalSec !== "number") d.estado.transCalSec = 5;
    return d;
  } catch {
    return null;
  }
...
export function montarSesion(ctx: Ctx, nav: Nav, plan: PlanSesion, estadoInicial?: RunnerState): () => void {
  ...
  sesionViva = true;
  if (hayWakeLockNativo()) void pedirWake();
  else activarVideoDespierta();
