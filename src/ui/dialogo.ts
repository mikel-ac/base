...
function montar<T>(
  cuerpoHTML: string,
  resolverDesde: (velo: HTMLElement, cerrar: (v: T) => void) => void,
  valorAlCancelar: T
): Promise<T> {
  ...
  if (hayDialogoAbierto) return Promise.resolve(valorAlCancelar);
  ...
  return new Promise<T>((resolve) => {
    ...
    const cerrar = (valor: T): void => {
      ...
      if (historialPropio && !porAtras) history.back();
      velo.remove();
      resolve(valor);
    };
    ...
    const primerCampo = velo.querySelector<HTMLInputElement>("input");
    const botonPrincipal = velo.querySelector<HTMLElement>("[data-dlg='ok']");
    (primerCampo ?? botonPrincipal)?.focus();
    primerCampo?.select();
  });
}

function pie(aceptar: string, cancelar: string, peligro = false): string {
  return `
    <div class="dlg-pie">
...
export function confirmar(o: OpcionesConfirmar): Promise<boolean> {
  ...
  return montar<boolean>(
    html,
    (velo, cerrar) => {
      velo.addEventListener("click", (ev) => {
        const b = (ev.target as HTMLElement).closest<HTMLElement>("[data-dlg]");
        if (!b) return;
...
export function confirmarResumen(o: OpcionesResumen): Promise<boolean> {
  const filas = o.filas
    .map(
      (f) => `
      <div class="dlg-fila">
 ...
 */
export function pedirTexto(o: OpcionesTexto): Promise<string | null> {
  const html = `
  ...
  return montar<string | null>(
    html,
    (velo, cerrar) => {
      const campo = velo.querySelector<HTMLInputElement>(".dlg-campo")!;
      const aceptar = (): void => cerrar(campo.value);
      velo.addEventListener("click", (ev) => {
        const b = (ev.target as HTMLElement).closest<HTMLElement>("[data-dlg]");
        if (!b) return;
        if (b.dataset["dlg"] === "ok") aceptar();
        else cerrar(null);
      });
