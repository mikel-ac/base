import type { Metricas, Tendencia, VolumenSemana } from "../entities/metricas.js";
...
export function calcularMetricas(usuario: Usuario, sesiones: Sesion[], ahora: number): Metricas {
  const ordenadas = [...sesiones].sort((a, b) => a.ts - b.ts);

  // --- sesiones por semana ISO ---
  const porSemana = new Map<string, { sesiones: number; minutos: number }>();
  ...
  for (const [, datos] of porSemana) {
    if (datos.sesiones >= usuario.objetivoSemanal) semanasCumplidas++;
  }

  // --- distribución de valoraciones (últimas N valoradas) ---
  const distribucion: Record<Valoracion, number> = { facil: 0, en_su_punto: 0, dura: 0 };
  const valoradas = ordenadas.filter((s) => s.valoracion !== null).slice(-N_VALORACIONES);
  for (const s of valoradas) distribucion[s.valoracion as Valoracion]++;

  // --- volumen por semana, con ceros, listo para una gráfica ---
  const volumenPorSemana: VolumenSemana[] = ultimasSemanasISO(ahora, N_SEMANAS_VOLUMEN).map(
    (clave) => ({
      semanaISO: clave,
  ...
  const cuenta = (claves: string[]) =>
    claves.reduce((suma, c) => suma + (porSemana.get(c)?.sesiones ?? 0), 0);
  const antiguas = cuenta(completas.slice(0, 3)) / 3;
  const recientes = cuenta(completas.slice(3, 6)) / 3;
  let tendenciaSesiones: Tendencia = "estable";
  if (recientes - antiguas >= 0.5) tendenciaSesiones = "subiendo";
  else if (antiguas - recientes >= 0.5) tendenciaSesiones = "bajando";
...
export class CalcularMetricas {
  ...
  async ejecutar(usuario: Usuario, ahora: number = Date.now()): Promise<Metricas> {
