 ...
 */
export function materialOk(e: Ejercicio, disponibles: Material[]): boolean {
  if (e.materiales.length === 0) return true;
  return e.materiales.some((m) => disponibles.includes(m));
}

/** ¿El ejercicio evita todas las zonas con molestia? (joints ∩ molestias = ∅) */
export function sinMolestias(e: Ejercicio, molestias: Zona[]): boolean {
  if (molestias.length === 0) return true;
...
export function impactoDeVariante(e: Ejercicio, v: Variante): Impacto {
 ...
 */
export function variantesPermitidas(e: Ejercicio, bajoImpacto: boolean): Variante[] {
  if (!bajoImpacto) return e.variantes;
 ...
 */
export function varianteParaNivel(
  e: Ejercicio,
  nivel: number,
  bajoImpacto: boolean
): Variante | null {
...
export function filtrarPool(catalogo: Ejercicio[], cfg: ConfigSesion): Ejercicio[] {
  const enZona = (e: Ejercicio) =>
    cfg.patrones === undefined || cfg.patrones.length === 0 || cfg.patrones.includes(e.patron);
  return catalogo.filter(
    (e) => e.tipo !== "calentamiento" && cfg.focus.includes(e.tipo) && enZona(e) && esViable(e, cfg)
  );
}

/** Pool del calentamiento: tipo "calentamiento" + viable (mismas molestias/material). */
export function filtrarCalentamientos(catalogo: Ejercicio[], cfg: ConfigSesion): Ejercicio[] {
