...
import { crearRunner, reducirRunner, type RunnerState } from "../state/runner.js";
...
if (!imposible.ok) comprobar(imposible.error.length > 0, "el fallo trae mensaje");
...
for (let semilla = 1; semilla <= 200; semilla++) {
  ...
  const molestias = semilla % 3 === 0 ? (["rodilla"] as const) : semilla % 3 === 1 ? (["hombro", "lumbar"] as const) : ([] as const);
  ...
  for (const a of [...plan.calentamiento, ...plan.principal]) {
    comprobar(!a.ejercicio.joints.some((z) => (cfg.molestias as string[]).includes(z)), `sesión ${semilla}: sin zonas con molestia (${a.ejercicio.id})`);
...
function sesionFalsa(desplDias: number, patrones: Sesion["patrones"], valoracion: Sesion["valoracion"]): Sesion {
  return {
...
if (planPrueba.ok) {
  let estado: RunnerState = crearRunner(planPrueba.valor, 3);
  ...
  for (let i = 0; i < 100000 && cursor.fase !== "fin"; i++) {
    ...
    cursor = paso.estado;
    if (paso.efectos.includes("AVISO_FIN")) avisosFin++;
  }
  ...
  while (cursor2.fase === "trabajo" && cursor2.indice === indiceMedido) {
    ...
    if (r2.efectos.includes("AVISO_CUENTA")) tics++;
